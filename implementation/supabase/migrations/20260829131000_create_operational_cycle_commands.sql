-- P1 operational cycles: a billable service remains a trip; related trips are
-- grouped explicitly for continuity/return visibility without merging money.

create type public.operational_cycle_leg_kind as enum ('outbound', 'return', 'continuation');

alter table public.operational_cycles
  add column version integer not null default 1 check (version > 0),
  add column idempotency_key uuid;

create unique index operational_cycles_company_idempotency_key_unique
  on public.operational_cycles (company_id, idempotency_key)
  where idempotency_key is not null;

alter table public.trips
  add column cycle_leg_kind public.operational_cycle_leg_kind,
  add column cycle_sequence integer;

alter table public.trips
  add constraint trips_cycle_leg_shape check (
    (cycle_id is null and cycle_leg_kind is null and cycle_sequence is null)
    or (
      cycle_id is not null
      and (
        (cycle_leg_kind is null and cycle_sequence is null)
        or (cycle_leg_kind is not null and cycle_sequence is not null and cycle_sequence > 0)
      )
    )
  );

create unique index trips_cycle_sequence_unique
  on public.trips (company_id, cycle_id, cycle_sequence)
  where cycle_id is not null and cycle_sequence is not null;

-- A cycle is a governed aggregation, not editable master data. Keep reads under
-- the existing staff RLS policy, but make writes flow through small commands.
revoke insert, update on table public.operational_cycles from authenticated;
drop policy if exists operational_cycles_staff_insert on public.operational_cycles;
drop policy if exists operational_cycles_staff_update on public.operational_cycles;

create function public.create_operational_cycle(
  p_id uuid,
  p_code text,
  p_vehicle_id uuid,
...
begin
  ...
  perform pg_advisory_xact_lock(
    hashtextextended(company_id::text || ':operational-cycle:' || p_idempotency_key::text, 0)
  );
  select * into existing
  from public.operational_cycles c
  where c.company_id = company_id and c.idempotency_key = p_idempotency_key;
  if found then
  ...
  if not exists (
    ...
    where v.company_id = company_id and v.id = p_vehicle_id and v.active
  ) then
  ...
  if p_primary_driver_id is not null and not exists (
    ...
    where d.company_id = company_id and d.id = p_primary_driver_id and d.active
  ) then
  ...
  insert into public.operational_cycles (
    ...
    p_id, company_id, normalized_code, p_vehicle_id, p_primary_driver_id,
    'planned', p_return_status, normalized_notes, auth.uid(), p_idempotency_key
  ) returning * into result;

  perform private.write_audit(
    company_id, 'OPERATIONAL_CYCLE_CREATED', 'operational_cycle', result.id,
...
create function public.update_operational_cycle(
  p_cycle_id uuid,
...
begin
  ...
  from public.operational_cycles c
  where c.company_id = company_id and c.id = p_cycle_id
  for update;
  if not found then
  ...
  set status = p_status,
      ...
      version = c.version + 1,
      updated_at = now()
  where c.company_id = company_id and c.id = old_cycle.id
  returning * into result;

  perform private.write_audit(
    company_id, 'OPERATIONAL_CYCLE_UPDATED', 'operational_cycle', result.id,
...
create function public.add_trip_to_operational_cycle(
  p_cycle_id uuid,
  p_trip_id uuid,
...
begin
  ...
  from public.trips t
  where t.company_id = company_id and t.id = p_trip_id
  for update;
  if not found then
  ...
  from public.trips t
  where t.company_id = company_id and t.cycle_id = cycle_row.id;
  ...
  set cycle_id = cycle_row.id,
      ...
      version = t.version + 1,
      updated_at = now()
  where t.company_id = company_id and t.id = old_trip.id
  returning * into result;
  update public.operational_cycles c
  set version = c.version + 1, updated_at = now()
  where c.company_id = company_id and c.id = cycle_row.id;

  perform private.write_audit(
    company_id, 'TRIP_ADDED_TO_OPERATIONAL_CYCLE', 'trip', result.id,
    to_jsonb(old_trip), to_jsonb(result), null
  );
  perform private.write_audit(
    company_id, 'OPERATIONAL_CYCLE_TRIP_ADDED', 'operational_cycle', cycle_row.id,
...
create function public.remove_trip_from_operational_cycle(
  p_cycle_id uuid,
  p_trip_id uuid,
  p_expected_cycle_version integer,
  p_reason text
...
begin
  ...
  perform private.write_audit(
    company_id, 'TRIP_REMOVED_FROM_OPERATIONAL_CYCLE', 'trip', result.id,
    to_jsonb(old_trip), to_jsonb(result), normalized_reason
  );
  perform private.write_audit(
    company_id, 'OPERATIONAL_CYCLE_TRIP_REMOVED', 'operational_cycle', cycle_row.id,
...
revoke all on function public.create_operational_cycle(uuid,text,uuid,uuid,public.return_status,text,uuid) from public, anon, authenticated, service_role;
revoke all on function public.update_operational_cycle(uuid,integer,public.operational_cycle_status,public.return_status,text) from public, anon, authenticated, service_role;
revoke all on function public.add_trip_to_operational_cycle(uuid,uuid,public.operational_cycle_leg_kind,integer) from public, anon, authenticated, service_role;
revoke all on function public.remove_trip_from_operational_cycle(uuid,uuid,integer,text) from public, anon, authenticated, service_role;

grant execute on function public.create_operational_cycle(uuid,text,uuid,uuid,public.return_status,text,uuid) to authenticated;
grant execute on function public.update_operational_cycle(uuid,integer,public.operational_cycle_status,public.return_status,text) to authenticated;
grant execute on function public.add_trip_to_operational_cycle(uuid,uuid,public.operational_cycle_leg_kind,integer) to authenticated;
grant execute on function public.remove_trip_from_operational_cycle(uuid,uuid,integer,text) to authenticated;
